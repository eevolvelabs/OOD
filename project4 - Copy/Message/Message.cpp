...
Message Message::FromString(const std::string& xml)
{
  ...
  while(rdr.next())
  {
    ...
    if(tag == "type")
    {
      //if(rdr.body() =="\"text\"")           /// ORGINAL
      if(rdr.body().compare("text") == 0)     // Modified Dhaval Malaviya
...
void main()
{
  ...
  msg2.getMsgType() = Message::file; //Message::MsgType::file;
  std::cout << "\n  " << msg2.ToString();
  testMessage(msg2);

  Message msg3("/* this is a C comment */");
  msg3.getMsgType() = Message::text; //Message::MsgType::text;
  std::cout << "\n  " << msg3.ToString();
  testMessage(msg3);
