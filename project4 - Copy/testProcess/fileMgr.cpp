...
{
  return directory::GetCurrentDirectoryA(); 
    //directory::GetCurrentDirectoryW();
}

// Member function to copy file from the source to destination
int fileMgr::copyFilesToWDir(std::string& src,std::string& desti)
{
  int okay = MoveFile(src.c_str(),desti.c_str());
  return okay;
}
