...
int main(int argc, char* argv[])
{
  ...
  //std::string inputXMLFileString;
  //std::cout << argv[1];

  //test1 = argv[1];
  std::string pathToXml = "..\\project2\\graphTest.xml";
  //createGraphFromXML(g4,pathToXml);
  //test1 = test1.c_str();

  std::ifstream inputXMLFileString (pathToXml);

  std::string content( (std::istreambuf_iterator<char>(inputXMLFileString) ),
                       (std::istreambuf_iterator<char>()    ) );

  XmlReader rdr(content);
  //XmlReader rdr(argv[1]);
